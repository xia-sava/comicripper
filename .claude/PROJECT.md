...
### バージョン
- 現在: 0.7.2（major-restructuring ブランチ）
- master: 0.6.6（安定版）
...
│   ├── model/Comic.kt              # ドメインモデル（画像キャッシュ・リスナー・スレッドセーフコレクション含む）
...
│   └── Setting.kt                  # アプリ設定シングルトン（JavaFX Properties）
├── repository/
...
- SimpleListProperty<Comic> による observable なインメモリストレージ
...
## 技術スタック
- Kotlin 2.1.21, JavaFX 21, TornadoFX 1.7.20
- Koin 4.0.0（DI）, kotlinx-coroutines-javafx 1.10.2
- JNotify 0.94（ファイル監視）, Jsoup 1.20.1（Webスクレイピング）, Gson 2.13.1
- Gradle 8.12, Shadow JAR
- テスト: JUnit5 + Koin Test
...
### 中期
1. JavaFX Property → Kotlin Flow 移行
2. ComicStorage シングルトン廃止 → StateFlow/MutableStateFlow
3. 外部API処理の改善・統合テスト

### 長期
1. Compose Desktop 移行（MVVM + Material3）
2. 画像処理の並列化・メモリ最適化
