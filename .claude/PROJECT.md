...
- バッチ内のイベントは常に「追加→削除」の順で通知するため、同名ファイルの削除→再作成が同じ200msに
  入ると「追加（重複なので無視）→削除」の順で届く。削除の通知ではディスクに残っているファイルを
  構成から外さない（`ComicRepository.removeDeletedFiles`）ことで、置き直されたファイルが一覧から
  消えないようにしている
